...
\title{
    \vspace{-2cm}
    \LARGE{\textbf{NeuraLeaf: An AI-Driven Sensor Fusion Retrofit Kit for Smart Tea Drying and Quality Optimization in Legacy Factories}}\\
}
\author{Harsh Mishra \quad Roll No. 240151008\\Hrimaan karigapsa \quad Roll No. 240151010}
\date{}
...
The specific challenges motivating NeuraLeaf include:
\begin{enumerate}
...
To accurately represent the thermochemical dynamics of tea drying without physical destructive testing, the NeuraLeaf Digital Twin employs a mathematical physics engine based on first-principles thermodynamics and Arrhenius reaction kinetics. All sub-models are implemented in Python and coupled through a unified time-stepping driver (\texttt{dryer.py}).
...
The NeuraLeaf system is organized as a three-layer architecture: Perception (sensors), Application (Digital Twin + UI), and Control (RL agent). Figure~\ref{fig:arch} illustrates the data flow across all layers.
...
\end{tikzpicture}
\caption{NeuraLeaf three-layer system architecture showing data flow from sensors through the Digital Twin to the RL controller and back.}
\label{fig:arch}
\end{figure}
...
By logging immutable data on thermodynamic efficiency, NeuraLeaf quantifies exact coal reductions:
\begin{itemize}
...
This project has successfully designed, implemented, and validated \textbf{NeuraLeaf}---an end-to-end AI-driven Digital Twin system for tea drying optimization. The key achievements are:
